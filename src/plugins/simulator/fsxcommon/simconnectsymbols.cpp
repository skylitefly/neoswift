...
#define SWIFT_SIMCONNECT_WRAPPER_API extern "C" __declspec(dllexport) HRESULT WINAPI
#define SIMCONNECTAPI SWIFT_SIMCONNECT_WRAPPER_API
#include "plugins/simulator/fsxcommon/simconnectwindows.h"
#undef SIMCONNECTAPI

// SimConnect.h saw SWIFT_SIMCONNECT_WRAPPER_API as SIMCONNECTAPI above, so the
// SDK declarations and our wrapper definitions below use the same linkage.
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_Open(HANDLE *phSimConnect, LPCSTR szName, HWND hWnd, DWORD UserEventWin32,
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_Text(HANDLE hSimConnect, SIMCONNECT_TEXT_TYPE type, float fTimeSeconds,
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_CallDispatch(HANDLE hSimConnect, DispatchProc pfcnDispatch, void *pContext)
{
    return gSymbols.SimConnect_CallDispatch(hSimConnect, pfcnDispatch, pContext);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_WeatherSetModeCustom(HANDLE hSimConnect)
{
    return gSymbols.SimConnect_WeatherSetModeCustom(hSimConnect);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_WeatherSetModeGlobal(HANDLE hSimConnect)
{
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_TransmitClientEvent(HANDLE hSimConnect, SIMCONNECT_OBJECT_ID ObjectID,
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_SubscribeToFacilities(HANDLE hSimConnect, SIMCONNECT_FACILITY_LIST_TYPE type,
                                                              SIMCONNECT_DATA_REQUEST_ID RequestID)
{
    return gSymbols.SimConnect_SubscribeToFacilities(hSimConnect, type, RequestID);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_GetLastSentPacketID(HANDLE hSimConnect, DWORD *pdwError)
{
    return gSymbols.SimConnect_GetLastSentPacketID(hSimConnect, pdwError);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_AIRemoveObject(HANDLE hSimConnect, SIMCONNECT_OBJECT_ID ObjectID,
                                                       SIMCONNECT_DATA_REQUEST_ID RequestID)
{
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_AIReleaseControl(HANDLE hSimConnect, SIMCONNECT_OBJECT_ID ObjectID,
                                                         SIMCONNECT_DATA_REQUEST_ID RequestID)
{
    return gSymbols.SimConnect_AIReleaseControl(hSimConnect, ObjectID, RequestID);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_AICreateNonATCAircraft(HANDLE hSimConnect, const char *szContainerTitle,
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_MapClientDataNameToID(HANDLE hSimConnect, const char *szClientDataName,
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_ChangeView(HANDLE hSimConnect, const char *szName)
{
    return gSymbols.SimConnect_ChangeView(hSimConnect, szName);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_AIReleaseControlEx(HANDLE hSimConnect, SIMCONNECT_OBJECT_ID ObjectID,
...
SWIFT_SIMCONNECT_WRAPPER_API SimConnect_ObserverAttachToEntityOn(HANDLE hSimConnect, const char *szName,
                                                                 DWORD dwObjectID, SIMCONNECT_DATA_XYZ Offset)
{
    return gSymbols.SimConnect_ObserverAttachToEntityOn(hSimConnect, szName, dwObjectID, Offset);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_CreateObserver(HANDLE hSimConnect, const char *szName,
                                                       SIMCONNECT_DATA_OBSERVER ObserverData)
{
    return gSymbols.SimConnect_CreateObserver(hSimConnect, szName, ObserverData);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_OpenView(HANDLE hSimConnect, const char *szName, const char *szTitle)
{
    return gSymbols.SimConnect_OpenView(hSimConnect, szName, szTitle);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_CloseView(HANDLE hSimConnect, const char *szName)
{
    return gSymbols.SimConnect_CloseView(hSimConnect, szName);
}

SWIFT_SIMCONNECT_WRAPPER_API SimConnect_SetObserverLookAt(HANDLE hSimConnect, const char *szName,
